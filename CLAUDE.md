...
**ScanProfile / RuleCatalog** — four profiles (`baseline`, `developer`, `designer`, `video-builder`) exist for the CLI. The SwiftUI app uses `RuleCatalog.all`, which unions all profiles (**35 unique rules**, deduplicated by rule ID) so every category is always scanned in one pass. There is no profile picker in the app. `FullDiskAccessChecker` heuristics surface first-run coaching when protected Library paths are unreadable.
...
This project is indexed by GitNexus as **pare** (822 symbols, 828 relationships, 0 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
