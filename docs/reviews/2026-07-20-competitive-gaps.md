...
**Date:** 2026-07-20  
**Inputs:** Current Pare master (Phases 1–8 + Leave Homebrew #24), refreshed [App A comparison](../features/comparison-app-a.md), new [App B comparison](../features/cleanmymac-comparison.md), open [US-4](../user-stories.md).

---

## Executive summary

Pare has **closed most historical App A feature gaps** (app uninstall, AI/Homebrew caches, project purge, installer finder, browser extended, history UI, maintenance subset, disk tree). Competing with **App B** is not about matching malware or Space Lens — it is about **shipping**, **first-run trust**, and a **simpler safe-clean journey**.
...
| One-click care UX | Ahead of CLI | **Behind** | No Smart Care analogue |
| App + Homebrew lifecycle | Ahead | Ahead on Brew | Leave Homebrew is unique |
| Disk visualization | Behind TUI polish | **Behind** Space Lens | Good enough for v1 |
...
| | |
|--|--|
| **Why #1** | Without a notarized DMG, competitive comparisons are academic — only clone-and-build users exist. App B and even App A (via brew) install in minutes. |
...
### 3. “Safe Care” one-pass journey (Smart Care, Pare-shaped)
...
| **Scope** | Home/hero action: Run Safe Care → unified scan → auto-select `.safe` only → confirm sheet (counts + size + large-clean Spotlight heads-up) → clean to Trash → optional “Run light maintenance?” (DNS only or user-picked) · Never auto-include `.review` / `.advanced` |
| **Effort** | Medium |
...
| **Scope** | Fix pre-existing failing tests (e.g. HomebrewCacheRule age/folder expectations post reconstructible-cache changes) · Align CLAUDE/CHANGELOG rule counts · Rename `PareApp.swift` → `PareApp.swift` · DiagnosticsExporter if not done in #1 · Manual matrix notes for Maintenance on current macOS |
| **Effort** | Small–medium |
...
| Menu bar monitor | App A `status` / App B Menu — not reclaim core |
...
| Doc | Role |
|-----|------|
| [comparison-app-a.md](../features/comparison-app-a.md) | Updated App A matrix (mostly closed gaps) |
| [cleanmymac-comparison.md](../features/cleanmymac-comparison.md) | New App B module map + do-not-build list |
...
**Stop cloning App A. Ship notarized Pare, teach Full Disk Access, and wrap existing safe clean into a single Safe Care journey — then harden tests and scan progress.**
