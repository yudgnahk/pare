...
## 2. Chrome reclaimable space vs CleanMyMac / Mole

### Local snapshot (this machine)

| Path | Size | Mole | Pare today |
|------|------|------|------------|
...
**Mole browser slice:** hundreds of MB to **~1 GB+** (Cache + Service Worker), when Chrome isn’t blocking.

### Why Pare under-reports Chrome

1. **3-day minimum age on browser / user caches** (`ScanPolicy.defaultCacheMinAgeSeconds`)  
   Sample of Chrome Cache files today: **all recent (< 3 days)**. Active browsers never surface disk cache under the age gate — CleanMyMac/Mole still list them.
...
5. **Running Chrome** — Mole skips some App Support cleanup while Chrome is open; Pare doesn’t special-case this yet.
...
CleanMyMac often pushes **Cache + Service Worker** harder than Local Storage. Pare currently **highlights Local Storage** (review) while **missing the safe-ish bulk cache** — product message is inverted vs industry tools.

### Solutions (scan coverage)

| Priority | Change | Risk level | Notes |
|----------|--------|------------|--------|
| P0 | Exempt **browser disk cache** under `Library/Caches/*Chrome*` from age gate **or** use 0–1 day | SAFE | Matches Mole/CleanMyMac |
...
| P1 | Multi-profile paths (`Profile 1`, `Profile 3`, …) not only `Default` | — | Mole lists multi-profile SW |
| P2 | Keep Local Storage / Cookies / History as **REVIEW**, default **unchecked** in selective clean | REVIEW | Never Quick Clean |
| P2 | Warn if browser is running before cleaning App Support paths | UX | Like Mole skip |
...
## 3. Pare vs `mo clean --dry-run` (this machine)

### Headlines

| Tool | “Potential” | Character |
|------|-------------|-----------|
| **Mole** `mo clean --dry-run` | **~5.36 GB**, 183 items, 37 categories | System + app caches, browsers, npm, Homebrew; **whitelists** JetBrains AS, Gradle, HuggingFace, etc. |
...
Numbers are not apples-to-apples: Mole skips whitelisted paths Pare may still report; Pare counts large project `node_modules` / `.venv` / `target` that Mole leaves to `mo purge`.

### Where Mole finds space Pare misses (or under-counts)

| Mole item | Size (approx.) | Pare gap |
|-----------|----------------|----------|
...
### Where Pare finds space Mole soft-pedals

| Pare item | Size (approx.) | Note |
|-----------|----------------|------|
| Project artifacts (`.cache`, `node_modules`, `target`, `.venv`) | **~5.2 GB** | Mole: `mo purge` / large-file hints |
| `go-build` + module cache | **~900 MB** | Mole developer section partial |
| JetBrains plugins / wrong-platform stubs | **GBs review** | Mole **whitelists** JetBrains Application Support |
| Cargo registry | large | Overlap with Mole |
...
5. **pnpm store** — advisory only or dedicated “store prune” action (Mole shows as large-file clue, not auto).  
6. **Directory-level findings** for `Library/Caches/Google/Chrome` instead of millions of tiny files (performance + UI).  
7. **Parity report** CI script: run Mole dry-run JSON vs Pare report, diff top paths.
...
## 6. Scan animation — three steps (CleanMyMac-like)
...
CleanMyMac cycles icons/copy while working. Pare should map steps to **real rule batches** so the UI isn’t lying.
...
4. **Mole parity** is optional; Pare’s differentiator is **project + JetBrains honesty** without Mole’s hard whitelist.  
...
|-------|-------------|---------|
| **A** | Browser SAFE pack + age-gate tweak + Service Worker | Chrome reclaimable competitive with CleanMyMac |
...
| **E** | npm `_npx`, OpenCode, broader user caches | Close remaining Mole gaps |
...
# → Detailed list: ~/.config/mole/clean-list.txt
