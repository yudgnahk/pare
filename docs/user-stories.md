...
- **Core**: Move `sourceApp(for:)` attribution logic from `main.swift` into `App BCore` as a `ScanReportAnnotator` utility so the SwiftUI app and CLI share the same attribution rules.
